...
$('div.bslider img.slideshow').each(function () {//START LOOP SLIDERS
	...
	//get id from current slider
	var id = $('div.bslider img.slideshow').attr('id')
	...
	//LOAD IMAGES
	 //create image array
	image[id] = new Array();
	//alert(window.img[id].length)
	for (var i=1;i<img[id].length;i++) {//start loop img to image
		image[id][i] = new Image();
		image[id][i].src = window.img[i];
	...
	$('#'+id).on('mouseout',function () {//start stop on mouseenter
		timeout=setTimeout('slider(id)',window.speed);	
	})//end stop on mouseenter
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////script
											//if('1' in window.img[id]) {//start settings are set
	...
	window.slideshow = function (sid,simage) {//start slideshow function
		if (simage === 'start') {alert(sid);slider(sid);}
			...
			//execute
			$('#'+sid).attr('src',image[sid][step].src);
			//update bullet
				//remove old
				var obullet = step+1;
			...
			//restart slideshow
			timeout=setTimeout('slider(sid)',window.speed);	
